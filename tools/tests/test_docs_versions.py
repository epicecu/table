...
class DocumentationVersionsTest(unittest.TestCase):
    ...
    def test_builds_latest_and_release_urls(self) -> None:
        entries = version_entries(
            ...
            normalise_project_base("table"),
        )
        self.assertEqual(
            entries,
            [
                {"label": "Latest", "url": "https://epicecu.github.io/table/"},
                {
                    "label": "1.2.5",
                    "url": "https://epicecu.github.io/table/1.2.5/",
                },
            ],
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                selected = build_versioned_site(
                    root, output, "https://epicecu.github.io", "/table/"
                )
            ...
            self.assertEqual(
                ...
                [
                    ("/table/", "Latest", "main"),
                    ("/table/2.0.1/", "2.0.1", "2.0.1"),
                    ("/table/1.2.5/", "1.2.5", "1.2.5"),
                ],
            )
