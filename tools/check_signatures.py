...
def owned_files() -> list[Path]:
    files: list[Path] = []
    for base in (ROOT / "src", ROOT / "extras" / "nanopb" / "table", ROOT / "examples"):
        for path in base.rglob("*"):
            if path.suffix in SUFFIXES and not EXCLUDED_PARTS.intersection(path.parts) and ".pb." not in path.name:
                files.append(path)
    return sorted(files)
...
def main() -> int:
    if len(sys.argv) == 2 and sys.argv[1] == "--fix":
        ...
        print("Formatted owned Table function signatures onto one physical line.")
        return 0
    failed = False
    for path in owned_files():
    ...
    if failed:
        return 1
    print("Owned Table function signatures are on one physical line.")
    return 0
