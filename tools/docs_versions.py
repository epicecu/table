...
def build_checkout(
    ...
    environment.update(
        {
            "TABLE_DOCS_VERSION": label,
            "TABLE_DOCS_SOURCE_REF": source_ref,
            "TABLE_DOCS_VERSIONS": json.dumps(entries, separators=(",", ":")),
        }
    )
    run(
        [
...
def build_versioned_site(
    ...
    for version in versions:
        with tempfile.TemporaryDirectory(prefix=f"table-docs-{version.tag}-") as temporary:
...
def main() -> int:
    ...
    build_parser.add_argument("--project-base", default="/table/")
